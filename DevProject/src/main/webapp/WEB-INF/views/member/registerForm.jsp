...
<body>
	...
	<p>
		...
		<font color="red">서버 쪽 컨트롤러 메서드에서 @PathVariables를 사용하지 않는 경우 파라미터로 해당 값을 얻을 수 없다.(null)</font>
	</p>
	...
	<p>6) HTML Form 필드값이 술자일 경우에는 컨트롤러 매개변수 타입이 문자열이면 그대로 문자열 형태로 들어오는가?</p>
	...
	<form action="/registerMemberPassword" method="post">
		password : <input type="text" name="password"><br>
		<input type="submit" value="registerMemberPassword"><br>
	</form><br>
	...
	<p>5) 폼 셀렉테 박스 요소값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다.</p>
	...
	<form action="/registerUserCardList" method="post">
		...
		카드1 - 유효년원 : <input type="text" name="cardList[0].validMonth"><br>
		카드2 - 번호 : <input type="text" name="cardList[1].no"><br>
		카드2 - 유효년원 : <input type="text" name="cardList[1].validMonth"><br>
	...
	<p>2) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값은 MultipartFile 매개변수와 문자열 매개변수로 처리한다.</p>
	...
	<p>5) 여러 개의 파일업로드 폼 파일 요소값을 여러 개의 MultipartFile 매개변수로 처리한다.</p>
	...
	<p>7) 여러개의 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을 MultiFileMember 타입의 바바빈즈 매개변수로 처리한다.</p>
