...
public class JSPFmttagController {
	 ...
	 * 			기본 값은 number이다. currency는 속해있는 국가코드에 맞는 통화 형식이 부여된다.
	 * 
	 * 		
	 * 		2) <fmt:parseNumber>
	 * 		- 문자열을 숫자로 변환한다
	 ...
	 * 		tpye	|	String		|	value 속성의 문자열 타입을 지정
	 ...
	 * 		type		|	String			|	날씨, 시간 또는 둘 다 포맷팅 할지의 여부를 지정한다.
	...
	// 2) type 속성을 지정되지 않으면 기본값은 number이다.
	...
	public String home0402(Model model) {
		...
		//String dateValue = "2019년. 2월. 1일 (금)";	// style long 형태로 지정
		//String dateValue = "2019년. 2월. 1일 금요일";	// style full 형태로 지정
		// 각 dateStyle로 지정된 값이 페이지로 넘어가 parsing이 진행될 때, parseDate 내에
