...
public class BoardController {
	 ...
	 * 	- 클래스 레벨로 요청 경로를 지정하면 메서드 레벨에서 지정한 경로의 기본 경로로 취급한다.
	 * 	- 클래스 레벨의 요청 경로에 메서드 레벨의 요청 경로를 덧붙인 형태가 최종 경로가된다.
	 */
	 ...
	 * 
	 * 	- 해당 경로로 요청 시, board 폴더에 read.jsp를 찾아서 페이지를 출력해주는 거 같지만,
	 * 	    경로상으로만 보면 bard/read/boardNo라는 파일까지 찾게된다.
	 * 
...
//	}
	...
	/*
	 * 4. Params 매핑
	 * @RequestMapping(vale = "/board/get, method = RequestMethod.GET, params="register"")
	...
	/*
	 * 6. Content Type 매핑
	 * - 요청의 Content Type 헤더 값을 매핑 조건으로 지정하는 경우에는 cunsumes 속성을 사용한다.
	 */
	
	// ResponseEntity 응답 상태 코드와 값을 제네릭에 설정되어 있는 타입에 따라서 내 마음대로 내보낼 수 있다.
	// 응답 상태 코드 : HttpStatus라는 객체 안에 있는 여러 상태 코드를 선택해서 내보낼 수 있다.
	...
	// produces 속성값을 지원하지 않으면 기본값인 "application/json" 미디어 타입으로 지정된다.
