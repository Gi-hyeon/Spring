...
public class SecurityController {
	 ...
	 * 			   위도한 행위(수정, 삭제, 등록 등)을 특정 웹 사이트에 요청하게 하는 공격을 말한다.
	 * 
	 * 2. 스프링 시큐리티 설정
	 * 	
	 * 		환경 설정
	 ...
	 *				: <security:intercept-url pattern="/notice/register" access="hasRole('ROLE_ADMIN')"/> ...등으로 설정
	 *
	 *		화면 설정
	 ...
	 *		- 공지 게시판 목록 화면 ( 모두 접근 가능하도록 되어있다 : permitAll )
	 *		- 공지 게시판 등록 화면 ( 관리자권한을 가진 사용자만 접근 가능 : hasRole('ROLE_ADMIN') )
	 *			> 접근 제한에 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 이동한다. 
	 *
	 * 4. 로그인 처리
	 * - 메모리상에 아이디와 패스워드를 지정하고 로그인 처리한다.
	 ...
	 * 
	 * 				** 우리는 현재 kr.or.ddit를 기준으로 base 패키지로 설정되어있기 때문에 위와 같은 설정을 하지는 않는다.
	 * 				     프로젝트를 만들고 설정하는 과정에서 base-package를 나눠야하는 상황일 때는 위와 같은 설정이 필요할 수 있다.
	 * 				     왜냐하면 security 패키지 아래에 있는 해당 컨트롤러들을 반으로 등록해야할 필요가 있기 때문이다.
	 * 		
	 * 		- 스프링 시큐리티 설정
	 ...
	 * 
	 * 6. 사용자 정의 접근 거부 처리자
	 * - 접근 거부가 발생한 상황에 단순 메시지 처리 이상의 다양한 처리를 하고싶다면 AccessDeniedHandler를 직접 구현하여야 한다.
	 * 
	 * 		환경 설정
	 * 		- 스프링 시큐리티 설정
	 * 			> security-context.xml 설정
	 * 				- id가 customAccessDenied를 가지고 있는 빈을 등록한다
	 * 					: 빈 등록에 사용되느 클래스는 security 패키지 아래에 있는 CustomAccessDeniedHandler클래스
	 * 		
	 * 		접근 거부 처리자 클래스 정의
	 * 		- 사용자가 정의한 접근 거부 처리자
	 * 			> CustomAccessDeniedHandler 클래스 정의
	 * 				- AccessDeniedHandler 인터페이스를 참조받아서 handle 메서드를 재정의하여 사용한다.
	 * 				: 우리는 접근이 거부되었을 때 빈으로 등록해둔 CustomAccessDeniedHandler 클래스가 발동해
	 * 				   해당 메서드가 실행되고 response 내장객체를 활용하여 / accessError URI로 이동하여 접근 거부 시
	 ...
	 * 			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고,
	 * 			    회원 권한을 가진 계정으로 접근 시, 공지사항 게시판 등록 페이지는 관리자 권한만 접근 가능하므로
	 * 			    접근이 거부된다. 이때, access-denied-handler로 설정되어 있는 ref 속성에 부여된 클래스 메서드로 이동하고
	 * 			    해당 페이지에서 접근이 거부되었을 때 보여질 페이지의 정보가 나타난다.
	 * 
	 * 7. 사용자 정의 로그인 페이지
	 ...
	 * 		- 스프링 시큐리티 설정
	 * 			> security-context.xml
	 * 				- <security:form-login/> 삭제 (기본 로그인 페이지로 사용하는 게 아니기 때문에 주석)
	 * 				- <security:form-login login-page="/login"/> 설정
	 * 					: 사용자가 직접 만든 로그인 페이지로 이동할 '/login' URI를 가지고 있는 컨트롤러 메서드를 정의한다.
	 * 
	 * 		로그인 페이지 정의
	 * 		- 사용자가 정의한 로그인 컨트롤러
	 ...
	 * 
	 * 8. 로그인 성공 처리
	 * - 로그인을 성공한 후에 로그인 이력 로그를 기록하는 등의 동작을 하고싶은 경우가 있다.
	 *   이런 경우에 AuthenticationSuccessHandler이라는 인터페이스를 구현해서 로그인 성공 처리자로 지정할 수 이싿.
	 *   
	 *   환경 설정
	 *   - 스프링 시큐리티 설정
	 *   	> security-context.xml
	 ...
	 * 			   logout처리 페이지에서도 action 경로는 /logout으로 설정한다
	 * 
	 * 10. JDBC 이용한 인증/인가 처리
	 * - 지정한 형식으로 테이블을 생성하면 JDBC를 이용해서 인증/인가를 처리할 수 있다.
	 *   생성한 테이블은 사용자를 관리하는 테이블(users)과 권한을 관리하는 테이블 2개로 구성되어 있다.
	 *   
	 *   	데이터베이스 테이블 준비
	 ...
	 *   	비밀번호 암호화 처리기 클래스 정의
	 *   	- 비밀번호 암호화 처리기
	 *   	> 스프링 시큐리티 5버전 부터는 기본적으로 PasswordEncoder를 지정해야하는데,
	 *   	   제대로 하려면 생성된 사용자 테이블(users)에 비밀번호를 암호화하여 저장해야한다.
	 *   	   테스트를 위해서 생성한 데이터는 암호화를 처리하지 않고 로그인하면  당연히 로그인 에러가 발생할 것이다.(암호화된 비밀번호가 날라가는 게 아니라서)
	 *   	   그래서 암호화를 하지 않는 PasswordEncoder를 직접 구현하여 지정하면 로그인 시 암호화를 고려하지 않으므로 로그인이 정상적으로 이루어지는 걸 확인할 수 있다.
	 *   		- security/CustomNoOpPasswordEncoder 클래스 구현
	 *   
	 * 11. 사용자 테이블을 이용한 인증/인가 처리
	 * 스프링 시큐리티가 기본적으로  이용하는 테이블 구조를 그대로 생성해서 사용해도 되지만,
	 * 기존에 구축된 회원 테이블이 있따면 약간의 작업으로 기존 테이블을 활용할 수 있습니다.
	 * 		
	 * 		데이터베이스 테이블 준비
	 ...
	 * 			- encode() 메서드를 통해서 SHA-2방식의 8바이트 Hash 암호를 매번 램덤하게 생성한다. -> SHA-1 다털림~
	 * 			- 똑같은 비밀번호를 입력하더라도 암호화되는 문자열을 매번 다른 문자열을 반환한다.
	 * 
	 * 		> 비밀번호를 입력하면 암호화된 비밀번호로 인코딩되는데, 암호화된 비밀번호와 디비 테이블에 있는 암호화된 비밀번호가 일치한 지를 파악한 후,
	 * 		   일치하면 로그인 성공으로 다음 스탭을 진행한다.
	 * 			- BCryptPasswordEncoder 클래스의 encode() 메서드를 통해 만들어지느 암호화된 해쉬 다이제스트들은 입력한 비밀번호 문자에 해당하는
	 * 			   수입억개의 다이제스트들 중에서 일치하는 다이제스트가 존재할 경우 비밀번호의 일치를 보고 인증을 성공시켜준다.
	 * 
	 ...
	 * 		> 기존 memberMapper에서 사용중인 read를 기반으로 한 readById를 새롭게 재정의하여 사용
	 ...
	 * 		
	 * 14. 자동 로그인
	 * - 로그인하면 특정 시간 동안 다시 로그인할 필요가 없는 기능이다.
	 ...
	 * 		- JSESSION를 삭제 후, 다시 로그인을 진행하더라도 로그인 후 진행될 페이지가 정상적으로 나타나는 걸 확인할 수 있다.
